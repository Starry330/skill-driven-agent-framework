...
from agent_framework.agents import create_fea_agent
from agent_framework.config import get_settings
from agent_framework.core import Gateway


def build_llm() -> ChatOpenAI:
    llm_settings = get_settings().fea_llm
    if not llm_settings.api_key:
        raise RuntimeError(
            "missing FEA model api key in .env: set "
            "AGENT_FRAMEWORK_FEA_API_KEY, MIMO_API_KEY, API_KEY, or OPENAI_API_KEY"
        )
...
def start_fea_chat() -> None:
    gateway = Gateway()
    spec, tools = create_fea_agent(build_llm(), gateway.settings)
